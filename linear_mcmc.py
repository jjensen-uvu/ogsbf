...
def linMC(x, y, xerr, yerr, nwalkers=100, nsteps=1000, ignore=100):
    """
    Perform linear regression with Bayesian inference using Markov Chain Monte Carlo (MCMC) sampling.

    This function fits a linear model to the given data using MCMC to estimate the uncertainties of the slope
    and intercept. It utilizes the `emcee` library for sampling the posterior distribution of the model parameters.
    ...
    xerr : array_like
        The uncertainties in the x data points. These are used as weights in the fit.

    yerr : array_like
        The uncertainties in the y data points. These are used as weights in the fit.

    nwalkers : int, optional
        The number of MCMC walkers. Default is 100.

    nsteps : int, optional
        The number of steps to run the MCMC chain. Default is 1000.

    ignore : int, optional
        The number of initial steps to ignore as burn-in. Default is 100.

    Returns:
    --------
    m_mcmc : tuple
        A tuple containing the median estimate of the slope (m), the uncertainty on the positive side, 
        and the uncertainty on the negative side.

    b_mcmc : tuple
        A tuple containing the median estimate of the intercept (b), the uncertainty on the positive side, 
        and the uncertainty on the negative side.

    samples : ndarray
        The MCMC samples of the parameters, reshaped into a 2D array where each row corresponds to a sample 
        and each column corresponds to a parameter.

    Notes:
    -------
    The function uses a linear model `y = mx + b` and estimates the posterior distributions of the slope (m)
    and intercept (b) by sampling from the likelihood using the MCMC method. The `emcee` package is used for 
    the MCMC sampling.

    The `lnlike` and `lnprob` functions, which are used to compute the log-likelihood and log-posterior 
    respectively, must be defined elsewhere in the code for this function to work.
    """
